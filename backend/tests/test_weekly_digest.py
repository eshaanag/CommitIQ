...
def _mock_exec(results: list):
    ...
    side = []
    for r in results:
        m = MagicMock(); m.scalars.return_value.all.return_value = r; side.append(m)
    return side
...
class TestWeeklyDigest:
    ...
    async def test_aggregates_commits(self):
        ...
        commits = [
            MagicMock(insertions=100, deletions=20, files_changed=3, author_email="a@x.com", committed_at=now),
            MagicMock(insertions=50, deletions=10, files_changed=2, author_email="b@x.com", committed_at=now),
        ]
        db = AsyncMock(); db.get.return_value = MagicMock(name="r", repo_slug="r")
    ...
    async def test_regression_alerts(self):
        ...
        prev = [MagicMock(health_score=80.0, avg_complexity=5.0, churn_rate=0.05, computed_at=prev_time)]
        db = AsyncMock(); db.get.return_value = MagicMock(name="r", repo_slug="r")
