...
def _r(id, cat, sev, title, desc, impact, effort, **kw):
    ...
    return {"id": id, "category": cat, "severity": sev, "title": title,
            "description": desc, "impact": max(0, min(100, impact)),
            "effort": effort, "metric": kw.get("metric"),
            "current_value": kw.get("cur"), "target_value": kw.get("tgt"),
...
def _complexity(snaps):
    ...
    if lat.avg_complexity > 10:
        recs.append(_r("high_complexity", "complexity", "critical",
                        ...
                        85, "high", metric="avg_complexity", cur=f"{lat.avg_complexity:.1f}", tgt="≤ 5.0"))
    elif lat.avg_complexity > 6:
        recs.append(_r("moderate_complexity", "complexity", "medium",
                        "Complexity moderately elevated",
                        f"Complexity is {lat.avg_complexity:.1f}. Consider refactoring the most complex units.",
                        45, "medium", metric="avg_complexity", cur=f"{lat.avg_complexity:.1f}", tgt="≤ 5.0"))
    if len(snaps) >= 2 and lat.avg_complexity > snaps[1].avg_complexity * 1.25:
        ...
        recs.append(_r("complexity_spike", "complexity", "high",
                        "Complexity spike in latest commit",
                        f"Complexity jumped {pct:.0f}% ({snaps[1].avg_complexity:.1f} → {lat.avg_complexity:.1f}).",
                        70, "medium", metric="avg_complexity", cur=f"{lat.avg_complexity:.1f}", tgt=f"≤ {snaps[1].avg_complexity:.1f}"))
    return recs
...
def _bus_factor(bfs):
    ...
    if crit:
        ...
        recs.append(_r("bus_factor_critical", "bus_factor", "critical",
                        ...
                        80, "high", metric="bus_factor_min", cur=f"{len(crit)} critical", tgt="0 critical"))
    elif high:
        names = ", ".join(b.module_path for b in high[:3])
        recs.append(_r("bus_factor_high", "bus_factor", "high",
                        f"{len(high)} module(s) at high bus-factor risk",
                        f"{names} have limited contributor diversity. Schedule knowledge-sharing.",
                        55, "medium", metric="bus_factor_min", cur=f"{len(high)} high", tgt="0 high"))
    return recs
...
def _hotspots(snaps):
    ...
    if s.hotspot_persistence_score > 70:
        recs.append(_r("persistent_hotspots", "hotspots", "high",
                        ...
                        60, "high", metric="hotspot_persistence_score", cur=f"{s.hotspot_persistence_score:.0f}", tgt="< 40"))
    elif s.hotspot_count > 5:
        recs.append(_r("many_hotspots", "hotspots", "medium",
                        f"{s.hotspot_count} hotspot files detected",
                        "Multiple high-churn complexity files. Prioritise the top 3 by risk score.",
                        45, "medium", metric="hotspot_count", cur=str(s.hotspot_count), tgt="< 3"))
    return recs
...
def _dependencies(snaps):
    ...
    recs = []
    if s.has_cycles:
        recs.append(_r("dependency_cycles", "dependencies", "critical",
                        "Circular dependency detected",
                        "Import cycles cause tight coupling and slow builds. Extract shared interfaces.",
                        85, "high", metric="has_cycles", cur="true", tgt="false"))
    if s.dependency_density > 0.6:
        recs.append(_r("high_dep_density", "dependencies", "high",
                        "High dependency density",
                        f"Density is {s.dependency_density:.2f}. High coupling makes changes risky.",
                        55, "high", metric="dependency_density", cur=f"{s.dependency_density:.2f}", tgt="< 0.4"))
    return recs
...
def _team_health(commits):
    ...
    ah = sum(1 for c in commits if c.committed_at and (c.committed_at.hour >= 20 or c.committed_at.hour < 8))
    n = len(commits)
    wp, ap = wk / n * 100, ah / n * 100
    recs = []
    if wp > 15 or ap > 20:
        recs.append(_r("burnout_risk", "team_health", "critical",
                        "Burnout risk: excessive after-hours commits",
                        f"{wp:.0f}% weekend, {ap:.0f}% after-hours. Predicts burnout and attrition.",
                        80, "medium", metric="burnout_risk", cur="High", tgt="Low"))
    elif wp > 5 or ap > 10:
        recs.append(_r("burnout_warning", "team_health", "high",
                        "Elevated after-hours commit activity",
                        f"{wp:.0f}% weekend, {ap:.0f}% after-hours. Monitor capacity.",
                        50, "low", metric="burnout_risk", cur="Medium", tgt="Low"))
    af = defaultdict(int)
    for c in commits:
    ...
    if af:
        ...
        if avg > 40:
            recs.append(_r("context_switching", "team_health", "high",
                            "High context-switching load",
                            f"Contributors average {avg:.0f} files/day. Consider feature ownership.",
                            50, "medium", metric="context_switching", cur="High", tgt="Low"))
    return recs
...
async def generate_recommendations(db: AsyncSession, repo_id: int) -> dict[str, Any]:
    ...
    snaps = (await db.execute(
        select(HealthSnapshot).where(HealthSnapshot.repo_id == repo_id)
        .order_by(HealthSnapshot.computed_at.desc()).limit(20)
    )).scalars().all()
    bfs = (await db.execute(
        select(BusFactor).where(BusFactor.repo_id == repo_id)
    )).scalars().all()
    commits = (await db.execute(
        select(Commit).where(Commit.repo_id == repo_id)
        .order_by(Commit.committed_at.desc()).limit(200)
    )).scalars().all()

    all_recs = (
        _health_trend(snaps) + _complexity(snaps) + _bus_factor(bfs) +
        _churn(snaps) + _hotspots(snaps) + _dependencies(snaps) +
        _team_health(commits) + _semantic_drift(snaps)
    )
    ...
    return {
        "repo_name": repo.name, "repo_slug": repo.repo_slug,
        ...
        "medium_count": med, "low_count": len(unique) - crit - high - med,
